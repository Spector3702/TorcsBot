...
class TorcsEnv:
    ...
    def step(self, u):
        ...
        #sp = np.array(obs['speedX'])/200  #CHANGECHANGECHANGECHANGECHANGE
        sp = np.array(obs['speedX'])
        
        progress = sp*np.cos(obs['angle']) - np.abs(sp*np.sin(obs['angle']))\
                    - sp*max(0, np.abs(obs['trackPos'])-0.5) #*np.sin(obs['angle'])
        #CHANGECHANGEOBVIOUSCHANGE
        #progress = sp*np.cos(obs['angle'])
        #reward = progress - sp*np.sin(obs["angle"]) - sp * np.abs(obs['trackPos'])
        reward = progress

        #ADDED THIS PART CHANGE CHANGE CHANGE
        if (np.abs(track.any()) > 1 or np.abs(obs['trackPos']) > 1):  # Episode is terminated if the car is out of track
           #reward = -200
        ...
        if obs['damage'] - obs_pre['damage'] > 0:
            info["collision"] = True
            #reward = -10 #WAS -1 CHANGE CHANGE CHANGE
        ...
        if np.cos(obs['angle']) < 0:  # Episode is terminated if the agent runs backward
            if self.time_step >  20 :
                ...
                # client.R.d['meta'] = True
        #if self.time_step % 50 -> Add positional reward mid-race 


        info["place"] = int(obs["racePos"])
        if episode_terminate is True: # Send a reset signal
            #reward += (obs["racePos"] == 1)*500 # If terminated and first place (WAS *20 CHANGECHANGE)
